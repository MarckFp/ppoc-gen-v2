    ...
    rsx! {
            ...
            div { class: "w-full max-w-2xl mx-auto space-y-5",
                ...
                div { class: "rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm p-4 sm:p-5 space-y-4",
                    div { class: "flex flex-col sm:flex-row gap-2 sm:items-center sm:justify-between",
                        ...
                        input { class: "h-10 w-full sm:w-64 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500", placeholder: t("common.search_placeholder"), value: query.read().clone(), oninput: move |e| { query.set(e.value()); apply_filter(); current_page.set(0); } }
                    }
                    {
                        ...
                        rsx!(
                            ...
                            // list
                            {
                                if page_items.is_empty() {
                                    rsx!( div { class: "text-sm text-slate-600 dark:text-slate-300", {t("absences.empty")} } )
                                } else {
                                    rsx!(
                                            ...
                                            for p in page_items.into_iter() {
                                                li { class: "py-3 flex items-center justify-between gap-3",
                                                    div { class: "flex items-center gap-3 min-w-0 w-full",
                                                        { select_mode().then(|| rsx!( input { r#type: "checkbox", checked: selected.read().contains(&p.id), onchange: move |_| {
                                                            let mut v = selected.read().clone(); if let Some(pos) = v.iter().position(|x| *x == p.id) { v.remove(pos); } else { v.push(p.id); } selected.set(v);
                                                        } } )) }
                                                        div { class: "min-w-0 flex-1 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/30 rounded-md px-3 -mx-3 py-2",
                                                            onclick: move |_| {
                                                                ...
                                                                #[cfg(all(feature = "native-db", not(target_arch = "wasm32")))]
                                                                {
                                                                    ...
                                                                    if let Ok(items) = dao::list_future_absences(t) {
                                                                        if let Some(a) = items.into_iter().find(|x| x.id == p.id) {
                                                                            form.set(AbsenceForm { id: Some(a.id), publisher_id: a.publisher_id.to_string(), start_date: a.start_date.to_string(), end_date: a.end_date.to_string(), description: a.description.unwrap_or_default() });
                                                                            modal_open.set(true);
                                                                        }
                                                                    }
                                                                }
                                                                ...
                                                                {
                                                                    let now = js_sys::Date::new_0();
                                                                    let t = format!("{:04}-{:02}-{:02}", now.get_full_year() as i32, now.get_month() as u32 + 1, now.get_date() as u32);
                                                                    if let Some(a) = wasm_backend::list_future_absences(&t).into_iter().find(|x| x.id == p.id) {
                                                                        form.set(AbsenceForm { id: Some(a.id), publisher_id: a.publisher_id.to_string(), start_date: a.start_date.to_string(), end_date: a.end_date.to_string(), description: a.description.unwrap_or_default() });
                                                                        modal_open.set(true);
                                                                    }
                                                                }
        ...
        { modal_open().then(|| rsx!(
            div { class: "fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4",
                div { class: "w-full max-w-md rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-lg p-5 space-y-4",
                    h2 { class: "text-lg font-semibold", { if form.read().id.is_some() { t("absences.edit_title") } else { t("absences.new_title") } } }
                    { error.read().as_ref().map(|err| rsx!( p { class: "text-red-600 text-sm", {err.clone()} } )) }
                    div { class: "grid grid-cols-1 gap-3",
                        select { class: "h-10 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500", value: form.read().publisher_id.clone(), onchange: move |e| form.write().publisher_id = e.value(),
                            option { value: "", selected: form.read().publisher_id.is_empty(), {t("absences.select_publisher")} }
                            for (id, name) in publishers.read().iter().cloned() {
                                option { value: "{id}", selected: form.read().publisher_id == id.to_string(), "{name}" }
                            }
                        }
                        input { r#type: "date", class: "h-10 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500", placeholder: {t("absences.start_date")}, value: form.read().start_date.clone(), oninput: move |e| form.write().start_date = e.value() }
                        input { r#type: "date", class: "h-10 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500", placeholder: {t("absences.end_date")}, value: form.read().end_date.clone(), oninput: move |e| form.write().end_date = e.value() }
                        textarea { class: "rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-full h-20", placeholder: {t("absences.description_optional")}, value: form.read().description.clone(), oninput: move |e| form.write().description = e.value() }
                    }
        ...
        { confirm_action.read().as_ref().map(|action| {
            let message = match action {
                ...
                ConfirmAction::DeleteMany(ids) => format!("{} ({})", t("absences.confirm_delete_many"), ids.len()),
            };
            rsx!(
                div { class: "fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4",
                    div { class: "w-full max-w-md rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-lg p-5 space-y-4",
                        ...
                        div { class: "flex items-center justify-end gap-2",
                            ...
                            button { class: "inline-flex items-center h-9 px-3 rounded-md bg-red-600 hover:bg-red-500 text-white text-sm font-medium transition",
                                onclick: move |_| {
                                    ...
                                    match act {
                                        Some(ConfirmAction::DeleteOne(id)) => { delete_absence(id); let mut v = selected.read().clone(); v.retain(|x| *x != id); selected.set(v); }
                                        Some(ConfirmAction::DeleteMany(ids)) => {
                                            let _ = &ids; // ensure used across cfg variants
                                            #[cfg(all(feature = "native-db", not(target_arch = "wasm32")))]
                                            { for id in ids.iter().copied() { let _ = dao::delete_absence(id); } let t = chrono::Local::now().date_naive(); if let Ok(items) = dao::list_future_absences(t) { let name_lookup = publishers.read().clone(); let mapped = items.into_iter().map(|a: NativeAbsence| { let name = name_lookup.iter().find(|(id, _)| *id == a.publisher_id).map(|(_, n)| n.clone()).unwrap_or_else(|| format!("#{:?}", a.publisher_id)); AbsenceItem { id: a.id, _publisher_id: a.publisher_id, title: format!("{} • {} → {}", name, format_date_ymd(&a.start_date.to_string()), format_date_ymd(&a.end_date.to_string())), subtitle: a.description.clone().unwrap_or_default() } }).collect::<Vec<_>>(); raw.set(mapped.clone()); list.set(mapped); } }
                                            #[cfg(target_arch = "wasm32")]
                                            { for id in ids.iter().copied() { wasm_backend::delete_absence(id); } let now = js_sys::Date::new_0(); let t = format!("{:04}-{:02}-{:02}", now.get_full_year() as i32, now.get_month() as u32 + 1, now.get_date() as u32); let name_lookup = publishers.read().clone(); let mapped = wasm_backend::list_future_absences(&t).into_iter().map(|a: WebAbsence| { let name = name_lookup.iter().find(|(id, _)| *id == a.publisher_id).map(|(_, n)| n.clone()).unwrap_or_else(|| format!("#{:?}", a.publisher_id)); AbsenceItem { id: a.id, _publisher_id: a.publisher_id, title: format!("{} • {} → {}", name, format_date_ymd(&a.start_date), format_date_ymd(&a.end_date)), subtitle: a.description.unwrap_or_default() } }).collect::<Vec<_>>(); raw.set(mapped.clone()); list.set(mapped); }
                                            selected.set(vec![]);
                                        }
                                        None => {}
                                    }
